...
def status_print(ip,port,thread_id,rps,path_get):
    print(f"{Fore.YELLOW}ZEEFU {Fore.LIGHTYELLOW_EX}HTTP {Fore.GREEN}-> {Fore.BLUE}TARGET{Fore.WHITE}={ip}:{port} {Fore.LIGHTBLUE_EX}PATH{Fore.WHITE}={path_get} {Fore.CYAN}RPS{Fore.WHITE}={rps} {Fore.LIGHTCYAN_EX}ID{Fore.WHITE}={thread_id}{Fore.RESET}")
